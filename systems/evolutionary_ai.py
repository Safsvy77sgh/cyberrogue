...
class Editor:
    ...
    def init_panels(self):
        ...
        self.tool_panel.add_button("Сохранить карту", lambda: self.save_map())
        self.tool_panel.add_button("Загрузить карту", lambda: self.load_map())
        self.tool_panel.add_button("Очистить", lambda: self.clear_all())
        self.panels.append(self.tool_panel)

        self.object_palette = EditorPanel("Палитра объектов", 270, 10, 300, 600)
    ...
    def serialize_objects(self) -> List[dict]:
        return [obj.to_dict() for obj in self.objects]

    def deserialize_objects(self, data: List[dict]):
...
# ============================================================
# ДОПОЛНИТЕЛЬНЫЙ КЛАСС ДЛЯ СОВМЕСТИМОСТИ
# ============================================================

class AIIndividual:
    """Особь для эволюционной системы."""
    def __init__(self):
        self.id = f"ai_{random.randint(1000, 9999)}"
        self.generation = 0
        self.parent_ids = []
        self.created_at = 0.0
        self.gene = BehaviorGene()
        self.rules = []
        self.total_encounters = 0
        self.total_damage_dealt = 0
        self.total_damage_taken = 0
        self.total_survival_time = 0.0
        self.total_player_resources_forced = 0
        self.total_deaths = 0
        self.fitness = 0.0
        self.last_fitness_update = 0.0
        self.player_weapon_history = []
        self.player_movement_patterns = []
        self.player_skill_patterns = []

    def calculate_fitness(self) -> float:
        if self.total_encounters == 0:
            return 0.0
        damage_score = self.total_damage_dealt * 0.4
        survival_score = self.total_survival_time * 0.2
        resource_score = self.total_player_resources_forced * 0.3
        death_penalty = self.total_deaths * 100
        self.fitness = damage_score + survival_score + resource_score - death_penalty
        return self.fitness
    ...
    def record_survival(self, time: float):
        self.total_survival_time += time

    def record_player_resource_use(self, amount: int):
        self.total_player_resources_forced += amount

    def record_death(self):
        self.total_deaths += 1

    def record_player_weapon(self, weapon: str):
        self.player_weapon_history.append(weapon)
        if len(self.player_weapon_history) > 20:
            self.player_weapon_history.pop(0)

    def record_player_movement(self, movement: str):
        self.player_movement_patterns.append(movement)
        if len(self.player_movement_patterns) > 50:
            self.player_movement_patterns.pop(0)

    def record_player_action(self, action: str):
        self.player_skill_patterns.append(action)
        if len(self.player_skill_patterns) > 50:
            self.player_skill_patterns.pop(0)

    def get_dominant_weapon(self) -> str:
        if not self.player_weapon_history:
            return 'pistol'
        return max(set(self.player_weapon_history), key=self.player_weapon_history.count)

    def get_movement_tendency(self) -> str:
        if not self.player_movement_patterns:
            return 'unknown'
        return max(set(self.player_movement_patterns), key=self.player_movement_patterns.count)

    def mutate(self, mutation_rate: float = 0.1):
        self.gene.mutate(mutation_rate)
        for rule in self.rules:
            rule.mutate(mutation_rate * 1.5)

    def crossover(self, other: 'AIIndividual') -> 'AIIndividual':
        child = AIIndividual()
        child.generation = max(self.generation, other.generation) + 1
        child.parent_ids = [self.id, other.id]
        child.gene = self.gene.crossover(other.gene)
        all_rules = self.rules + other.rules
        all_rules.sort(key=lambda r: r.get_success_rate(), reverse=True)
        child.rules = all_rules[:min(len(all_rules), 20)]
        child.mutate(0.05)
        return child
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'total_survival_time': self.total_survival_time,
            'total_player_resources_forced': self.total_player_resources_forced,
            'total_deaths': self.total_deaths,
            'fitness': self.fitness,
            'player_weapon_history': self.player_weapon_history,
            'player_movement_patterns': self.player_movement_patterns,
            'player_skill_patterns': self.player_skill_patterns,
        }

    @classmethod
    def from_dict(cls, data: dict) -> 'AIIndividual':
        individual = cls()
        individual.id = data.get('id', individual.id)
        individual.generation = data.get('generation', 0)
        individual.parent_ids = data.get('parent_ids', [])
        individual.created_at = data.get('created_at', 0.0)
        individual.gene = BehaviorGene.from_dict(data.get('gene', {}))
        individual.rules = [SituationalRule.from_dict(r) for r in data.get('rules', [])]
        individual.total_encounters = data.get('total_encounters', 0)
        individual.total_damage_dealt = data.get('total_damage_dealt', 0)
        individual.total_damage_taken = data.get('total_damage_taken', 0)
        individual.total_survival_time = data.get('total_survival_time', 0.0)
        individual.total_player_resources_forced = data.get('total_player_resources_forced', 0)
        individual.total_deaths = data.get('total_deaths', 0)
        individual.fitness = data.get('fitness', 0.0)
        individual.player_weapon_history = data.get('player_weapon_history', [])
        individual.player_movement_patterns = data.get('player_movement_patterns', [])
        individual.player_skill_patterns = data.get('player_skill_patterns', [])
        return individual
...
class BehaviorGene:
    """Ген поведения врага."""
    def __init__(self):
        self.optimal_attack_distance = 200.0
        self.min_attack_distance = 80.0
        self.max_attack_distance = 400.0
        self.aggression = 0.5
        self.retreat_threshold = 0.3
        self.chase_persistence = 0.6
        self.strafe_probability = 0.4
        self.strafe_direction_change = 2.0
        self.circle_strafe_probability = 0.3
        self.backpedal_probability = 0.3
        self.rush_probability = 0.3
        self.attack_when_player_reloading = 0.7
        self.attack_when_player_low_hp = 0.8
        self.attack_when_player_poisoned = 0.6
        self.attack_when_player_shielded = 0.2
        self.dodge_probability = 0.3
        self.block_probability = 0.2
        self.retreat_when_poisoned = 0.7
        self.retreat_when_low_hp = 0.8
        self.ability_usage_threshold = 0.5
        self.special_when_player_close = 0.6
        self.special_when_player_far = 0.3
        self.vs_shotgun_distance = 1.5
        self.vs_laser_distance = 1.2
        self.vs_pistol_distance = 0.8
    ...
    def mutate(self, mutation_rate: float = 0.1):
        for attr in self.__dict__:
            if random.random() < mutation_rate:
                ...
                if 'probability' in attr or 'threshold' in attr or attr.startswith('attack_') or attr.startswith('retreat_') or attr.startswith('dodge_') or attr.startswith('block_') or attr.startswith('special_') or 'aggression' in attr or 'persistence' in attr:
                    new_value = max(0.0, min(1.0, new_value))
                elif 'distance' in attr:
                    new_value = max(50.0, min(600.0, new_value))
                setattr(self, attr, new_value)

    def crossover(self, other: 'BehaviorGene') -> 'BehaviorGene':
        child = BehaviorGene()
        for attr in self.__dict__:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> 'BehaviorGene':
        gene = cls()
        gene.__dict__.update(data)
        return gene


class SituationalRule:
    """Ситуационное правило."""
    def __init__(self, condition: str = "", action: str = "", strength: float = 0.5, priority: float = 0.5):
    ...
    def mutate(self, mutation_rate: float = 0.2):
        if random.random() < mutation_rate:
            self.strength = max(0.0, min(1.0, self.strength + random.gauss(0, 0.1)))
        if random.random() < mutation_rate:
            self.priority = max(0.0, min(1.0, self.priority + random.gauss(0, 0.1)))

    def to_dict(self) -> dict:
        return self.__dict__.copy()

    @classmethod
    def from_dict(cls, data: dict) -> 'SituationalRule':
        rule = cls()
        rule.__dict__.update(data)
        return rule
...
# ============================================================
# МЕНЕДЖЕР ЭВОЛЮЦИОННОГО ИИ
# ============================================================

class EvolutionaryAIManager:
    """Менеджер эволюционного ИИ."""
    def __init__(self):
        self.population: List[AIIndividual] = []
        self.generation = 0
        self.save_file = "evolutionary_ai_data.json"
        self.mutation_rate = 0.1
        self.population_size = 50
        self.elite_percentage = 0.2
        self.evolution_active = False
        self.current_individual: Optional[AIIndividual] = None
    ...
    def _init_population(self):
        ...
        if not self.population:
            for _ in range(self.population_size):
                individual = AIIndividual()
                individual.generation = 0
                individual.rules = self._create_base_rules()
                individual.mutate(0.3)
                self.population.append(individual)

    def _create_base_rules(self) -> List[SituationalRule]:
        """Создание базовых правил."""
        rules = [
            SituationalRule('player_poisoned', 'keep_distance', 0.7, 0.8),
            SituationalRule('player_shotgun', 'keep_distance', 0.8, 0.9),
            SituationalRule('player_laser', 'dodge', 0.6, 0.7),
            SituationalRule('player_low_hp', 'rush', 0.7, 0.8),
            SituationalRule('player_shielded', 'retreat', 0.6, 0.7),
            SituationalRule('player_reloading', 'rush', 0.8, 0.9),
            SituationalRule('self_low_hp', 'retreat', 0.8, 0.9),
            SituationalRule('self_poisoned', 'retreat', 0.7, 0.8),
            SituationalRule('player_far', 'approach', 0.5, 0.5),
            SituationalRule('player_close', 'backpedal', 0.6, 0.7),
        ]
        return rules

    def get_current_generation(self) -> int:
        """Получение текущего поколения."""
        return self.generation

    def record_boss_defeat(self, wave: int, time: float):
        """Запись победы над боссом."""
        self.generation += 1
    ...
    def _evolve_population(self):
        ...
        elite_count = max(1, int(len(self.population) * self.elite_percentage))
        ...
        while len(new_population) < self.population_size:
            ...
            if parent1 != parent2:
                ...
                child.rules = [SituationalRule.from_dict(r.to_dict()) for r in parent1.rules]
                child.mutate(self.mutation_rate * 2)
    ...
    def get_best_individual(self) -> Optional[AIIndividual]:
        """Получение лучшей особи."""
        if not self.population:
            return None
        return max(self.population, key=lambda x: x.fitness)

    def select_individual_for_encounter(self, weapon: str = None) -> Optional[AIIndividual]:
        """Выбор особи для боя."""
        if not self.population:
            self._init_population()
        if weapon:
            adapted = [i for i in self.population if weapon in i.player_weapon_history]
            if adapted:
                return max(adapted, key=lambda x: x.fitness)
        return max(self.population, key=lambda x: x.fitness)

    def record_encounter_result(self, individual: AIIndividual, damage_dealt: int,
                               damage_taken: int, survival_time: float,
                               player_resources_used: int, died: bool):
        """Запись результата боя."""
        if individual:
            individual.record_damage_dealt(damage_dealt)
            individual.record_damage_taken(damage_taken)
            individual.record_survival(survival_time)
            individual.record_player_resource_use(player_resources_used)
            if died:
                individual.record_death()
            individual.calculate_fitness()

    def apply_behavior(self, enemy, player, game, dt: float):
        """Применение поведения особи к врагу."""
        if not self.current_individual:
            self.current_individual = self.select_individual_for_encounter()
        # Здесь можно применить генетические параметры к врагу
        # Например, агрессию, дистанцию атаки и т.д.
        if self.current_individual:
            gene = self.current_individual.gene
            enemy.aggression = gene.aggression
            if hasattr(enemy, 'optimal_attack_distance'):
                enemy.optimal_attack_distance = gene.optimal_attack_distance

    def get_population_stats(self) -> dict:
        """Получение статистики популяции."""
        if not self.population:
            return {}
        fitness_values = [i.fitness for i in self.population]
        return {
            'generation': self.generation,
            ...
            'best_individual': self.get_best_individual().id if self.get_best_individual() else None,
        }
    ...
    def load_population(self):
        ...
        if os.path.exists(self.save_file):
            try:
                ...
                self.population = [AIIndividual.from_dict(i) for i in data.get('population', [])]
            except:
                pass